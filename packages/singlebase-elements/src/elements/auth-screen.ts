...
export type ProtectedScreen = "account";
...
const PROTECTED_SCREENS: ProtectedScreen[] = ["account"];
 ...
 */
const flagAttr = {
  // An absent attribute must mean "default" (true), not false. Lit calls
  // fromAttribute(null) when an attribute is *removed*, so returning false
  // there would make removing `allow-oauth="false"` leave the flag off
  // forever instead of restoring the default.
...
export class SinglebaseAuthScreen extends SinglebaseElementBase {
  static override styles = [
    ...
    css`
      ...
      /* A flex column would stretch an image to full width and wreck its
         aspect ratio, so both logos opt out of the cross-axis stretch. */
      .logo-img {
        display: block;
      ...
      .brand-img {
        display: block;
        align-self: flex-start;
        height: var(--sb-brand-logo-height, 28px);
        width: auto;
        max-width: 100%;
      }
  ...
  /**
   * A logo image, used instead of `logo-text` when set. `logo-text` is still
   * worth supplying alongside it — it becomes the image's alt text.
   */
  ...
  /**
   * The "Auth by Singlebase" credit under the card. On by default; turn it
   * off with branding="false", exactly like the allow-* flags.
   *
   * It is a plain link — no pixel, no beacon, no request of any kind. A
   * third-party network call originating from a sign-in form is the sort of
  ...
  private get cfg() {
    ...
    // password-plus-OTP MFA (the auth API's code sign-in).
  ...
  private get activeScreen(): Screen {
    ...
    if (authenticated) {
      ...
      return askedForOne ? requested : "account";
    }
  ...
  private renderLogo() {
    if (this.logoUrl) {
      return html`<img class="logo-img" part="logo" src=${this.logoUrl} alt=${this.logoText} />`;
    }
    return this.logoText ? html`<div class="logo-dot" part="logo">${this.logoText}</div>` : nothing;
  }

  /** The larger logo in the split layout's brand panel. */
  private renderBrandMark() {
    if (this.logoUrl) {
      return html`<img class="brand-img" part="logo" src=${this.logoUrl} alt=${this.logoText} />`;
    }
    return this.logoText
      ? html`<div class="brand-mark" part="logo">${this.logoText}</div>`
      : nothing;
  }
  ...
  private panel(content: unknown) {
    return html`<div class="panel">${content}${this.renderBranding()}</div>`;
  }
    ...
    if (screen === "verify") {
      return html`
        ...
        <div class="resend-row">
          ...
          <button type="button" class="link-sm" ?disabled=${this.loading} @click=${this.onResend}>
            ${this.msg.resendCta}
          </button>
        </div>
      `;
  ...
  private renderGuest(screen: GuestScreen) {
    ...
    return html`
      <form class="stack" @submit=${this.onSubmit} novalidate>
        <div class="head">
          ${this.layout !== "split" ? this.renderLogo() : nothing}
  ...
  protected override render() {
    ...
    if (this.showsSignedInInterstitial) {
      if (this.noAccountView) return nothing;
      return this.panel(this.renderSignedIn());
    }
    ...
    return html`
      ...
      ${this.panel(blocked ? this.renderBlocked() : this.renderGuest(screen as GuestScreen))}
    `;
  }
}
