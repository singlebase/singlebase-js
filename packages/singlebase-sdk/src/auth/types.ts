...
/**
 * How authentication behaves, as opposed to where it connects.
 *
 * `SinglebaseClient` takes these under an `auth` key, so that options group by
 * domain — `auth` beside `authui` — rather than every package's settings
 * sharing one flat object. `createAuthClient` takes them inline, since at that
 * level they are the only options there are.
 */
...
export interface AuthClientOptions extends SinglebaseOptions, AuthOptions {}
